...
const CreateNote = ({ isOpen, onClose, onSaveNote, isEditingNote = null }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 flex flex-col h-screen w-screen overflow-hidden">
            <header className="h-16 px-6 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                    <button type="button" onClick={onClose} className="p-2.5 cursor-pointer text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-slate-800 dark:text-slate-200 hidden md:inline">StackNotes.</span>
                    </div>
                </div>

                <div className="flex items-center gap-3">
                    <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl">
                        Cancel
                    </button>
            ...
            className="flex-1 max-w-5xl w-full mx-auto p-6 flex flex-col gap-4 overflow-hidden">
                <input
                    ...
                    className="w-full text-2xl font-bold bg-transparent border-none outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-300"
                    autoFocus
                />

                <div className="flex items-center gap-3 flex-wrap pb-2 border-b border-slate-200 dark:border-slate-800">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-slate-400 uppercase">
                        <Tag className="w-3.5 h-3.5" />
                        <span>Category:</span>
                    </div>
                    <select name="category" value={formData.category} onChange={handleChange} className="px-3 py-1.5 text-sm rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 outline-none">
                    ...
                    disabled={!formData.category} className="px-3 py-1.5 text-sm rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 outline-none disabled:opacity-40">
